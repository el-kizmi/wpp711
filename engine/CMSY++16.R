...
# Dependency preflight. Installation belongs to deployment/setup, not concurrent jobs.
list.of.packages <- c("R2jags","coda","parallel","foreach","doParallel","gplots","mvtnorm","snpar","neuralnet","conicfit","pracma","geigen","LNPar")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) {
  stop("Missing CMSY++ R packages: ", paste(new.packages, collapse=", "),
       ". Install dependencies before starting a batch.")
}
...
if(!exists("n.chains")) n.chains <- 2 # number of chains to be used in JAGS, default = 2
if(!exists("parallel.chains")) parallel.chains <- FALSE
if(!exists("mcmc.iter")) mcmc.iter <- 60000
if(!exists("mcmc.burnin")) mcmc.burnin <- 30000
if(!exists("mcmc.thin")) mcmc.thin <- 10

# headless windows device replacement for non-interactive sessions
if (!interactive()) {
  windows <- function(...) {
    if (dev.cur() == 1) {
      pdf(NULL, width = 14, height = 9)
      dev.control('enable')
    }
  }
}

# set working directory dynamically to source file location or fallback to current directory
if (Sys.getenv("RSTUDIO") == "1") {
  path <- rstudioapi::getActiveDocumentContext()$path
  if (!is.null(path) && path != "") {
    tryCatch(setwd(dirname(path)), error = function(e) {
      if (dir.exists("terubuk")) setwd("terubuk")
    })
  } else {
    if (dir.exists("terubuk")) setwd("terubuk")
  }
  ...
  file.arg <- "--file="
  match <- grep(file.arg, args)
  if (length(match) > 0) {
    path <- sub(file.arg, "", args[match])
    setwd(dirname(path))
  } else {
    if (dir.exists("terubuk")) setwd("terubuk")
  }
...
#-----------------------------------------
if(!exists("catch_file")) catch_file  <- "Terubuk_data.csv"
if(!exists("id_file")) id_file     <- "Terubuk_ID.csv"
if(!exists("nn_file")) {
  if (file.exists("ffnn.bin")) {
    nn_file <- "ffnn.bin"
  } else if (file.exists("../ffnn.bin")) {
    nn_file <- "../ffnn.bin"
  } else {
    nn_file <- "ffnn.bin"
  }
}
if(!exists("outfile")) outfile     <- paste("Out_",format(Sys.Date(),format="%B%d%Y_"),id_file,sep="")

#----------------------------------------
# Select stock to be analyzed ----
#----------------------------------------
if(!exists("stocks")) stocks      <- NA

#-----------------------------------------
# General settings for the analysis ----
#-----------------------------------------
if(!exists("CV.C")) CV.C         <- 0.15
if(!exists("CV.cpue")) CV.cpue      <- 0.2
if(!exists("sigmaR")) sigmaR       <- 0.1
if(!exists("cor.log.rk")) cor.log.rk   <- -0.76
if(!exists("rk.cor.beta")) rk.cor.beta  <- c(2.52,3.37)
if(!exists("nbk")) nbk          <- 3
if(!exists("bt4pr")) bt4pr        <- FALSE
if(!exists("auto.start")) auto.start   <- FALSE
if(!exists("ct_MSY.lim")) ct_MSY.lim   <- 1
if(!exists("q.biomass.pr")) q.biomass.pr <- c(0.4,0.8)
if(!exists("n")) n            <- 5000
if(!exists("ni")) ni           <- 3
if(!exists("nab")) nab          <- 3
if(!exists("bw")) bw           <- 3
if(!exists("mgraphs")) mgraphs      <- TRUE
if(!exists("e.creep.line")) e.creep.line <- TRUE
if(!exists("kobe.plot")) kobe.plot    <- TRUE
if(!exists("BSMfits.plot")) BSMfits.plot <- TRUE
if(!exists("pp.plot")) pp.plot      <- TRUE
if(!exists("rk.diags")) rk.diags     <- TRUE
if(!exists("retros")) retros       <- TRUE
if(!exists("save.plots")) save.plots   <- TRUE
if(!exists("close.plots")) close.plots  <- TRUE
if(!exists("write.output")) write.output <- TRUE
if(!exists("write.pdf")) write.pdf    <- FALSE
if(!exists("select.yr")) select.yr    <- NA
if(!exists("write.rdata")) write.rdata  <- FALSE
...
bsm   <- function(ct,btj,nyr,prior.r,prior.k,startbio,q.priorj,
  ...
  # run model ----
  # Batch execution already parallelizes stocks. Sequential chains avoid nested
  # PSOCK clusters and intermittent Windows server-socket/port failures.
  jags_runner <- if(isTRUE(parallel.chains)) jags.parallel else jags
  jags_outputs <- jags_runner(data=jags.data,
                              working.directory=NULL, inits=j.inits,
                              parameters.to.save=jags.save.params,
                              model.file="r2jags.bug", n.chains = n.chains,
                              n.burnin = mcmc.burnin, n.thin = mcmc.thin,
                              n.iter = mcmc.iter)
  diag.summary <- jags_outputs$BUGSoutput$summary
  diag.rhat <- suppressWarnings(as.numeric(diag.summary[,"Rhat"]))
  diag.neff <- suppressWarnings(as.numeric(diag.summary[,"n.eff"]))
  max.rhat <- if(any(is.finite(diag.rhat))) max(diag.rhat[is.finite(diag.rhat)]) else NA_real_
  min.neff <- if(any(is.finite(diag.neff))) min(diag.neff[is.finite(diag.neff)]) else NA_real_
  cat(sprintf("MCMC_DIAGNOSTIC max_Rhat=%.4f min_n_eff=%.0f\n", max.rhat, min.neff))
  if(is.finite(max.rhat) && max.rhat > 1.1) warning("MCMC convergence warning: max Rhat > 1.1")
  return(jags_outputs)
}
...
for(stock in stocks) {
  ...
  # assign data from cinfo to vectors
    ...
    if(is.na(int.yr)==F) {
      # use int.yr adjusted/given by user
    ...
    n_last            <- min(10, nyr)
    ct.last           <- ct[(nyr-n_last+1):nyr]/mean(ct) # last catch standardized by mean catch
    yrs.last          <- seq(1:n_last)
    ...
    n_first           <- min(10, nyr)
    ct.first          <- ct[1:n_first]/mean.ct # catch standardized by mean catch
    yrs.first         <- seq(1:n_first)
